class Integer
  def segment
    ...
    chars = segments.split("\n").compact.map { _1.chars }
    ...
    chars[y, height].map { _1[x, width] }
  end

  def length
    chars.uniq.select { _1 =~ /[a-g]/ }.size
  end
...
def part1(notes)
  outputs = notes.flat_map { _2 }.select do |val|
    [1, 4, 7, 8].any? { _1.length == val.chars.length }
  end
...
notes = DATA.read
  .split("\n")
  .map { _1.split(/\|/) }
  .map { [_1[0].split(" "), _1[1].split(" ")] }

puts "part 1: %d" % part1(notes)
# puts "part 2: %d" % part2(notes)
